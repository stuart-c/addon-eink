...
pip install -q -r "$TOP_DIR/eink_layout_manager/backend/requirements.txt"
...
if [ -d "$TOP_DIR/eink_layout_manager/e2e_python" ]; then
    echo "--- Running Python E2E Tests ---"
    cd "$TOP_DIR"
    export PYTHONPATH=$TOP_DIR
    pytest eink_layout_manager/e2e_python
fi
