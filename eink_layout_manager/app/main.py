...
async def save_item(request):
    ...
    try:
        ...
        return web.json_response({"error": f"Schema for {resource_type} not found"}, status=500)
    except ValidationError as e:
        return web.json_response({"error": "Validation failed", "message": e.message}, status=400)
...
def init_app():
    ...
    app.router.add_post(f"{api_prefix}", save_item)
