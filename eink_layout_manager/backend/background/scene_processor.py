...
async def process_scene(scene, session):
    ...
    try:
        ...
        stmt = select(models.DisplayType).where(models.DisplayType.id.in_(display_type_ids))
        result = await session.execute(stmt)
        ...
        for item in scene.items:
            ...
            # Get pxPerMm from the FIRST display in the item (consistent with frontend)
            ...
            for image_meta in images_in_item:
                ...
                for panel in item_panels:
                    await process_slice(
                        scene, panel, image_meta, image_record, 
                        min_x, min_y, px_per_mm, session
                    )
...
async def process_slice(scene, panel, image_meta, image_record, item_min_x, item_min_y, px_per_mm, session):
    """Check and process a single slice for a (scene, panel, image) combination."""
    try:
        scene_id = scene.id
        display_id = panel["id"]
        ...
        if record:
            if record.scene_hash == scene.scene_hash and record.image_hash == image_record.settings_hash:
                # Up to date
                return
        
        # Need to run conversion
        logger.info(f"Generating slice for Scene {scene_id}, Display {display_id}, Image {image_id}")
        ...
        process = await asyncio.create_subprocess_exec(
            "node", converter_path, json.dumps(config),
        ...
        logger.exception(f"Failed to process slice for {scene.id}/{panel['id']}/{image_record.id}: {str(e)}")
